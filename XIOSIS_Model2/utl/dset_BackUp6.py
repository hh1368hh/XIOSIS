...
DataInfoTuple = namedtuple('DataInfoTuple','LFOVaddress, CTFaddress')
...
def getdatainfo(data_dir):
    ...
    for i,c in enumerate(CTF_fns):
        ...
        SFOV_add=(CasecolCTF + '_0.nii',CasecolCTF + '_1.nii', CasecolCTF + '_2.nii')
        ...
        LFOV_add=(SFOV_add[0].replace('CTF','LFOV'),SFOV_add[1].replace('CTF','LFOV'),SFOV_add[2].replace('CTF','LFOV'))
        # LFOV_add=LFOV_add.replace('projc','projp')
        # field_add=field_add.replace('image','field')
        #print(field_add)
        #print(image_fns[i])

        datainfolist.append(DataInfoTuple(LFOV_add,CTF_add))
    return datainfolist
...
def getData(filenames):
    ...
    inP=resize(inP,(3,128,128))
        
    outP=nib.load(CTF_fns).get_fdata().astype(np.float32)
    outP=resize(outP,(128,128,128))
