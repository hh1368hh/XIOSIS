...
class UnetValidatingApp:
    ...
    def __init__(self,val_dir='./Data'):
        ...
        self.model=self.initModel()
        self.model.netP=self.model.netP.module
    ...
    def initValDataset(self,tfms=None):
        ...
        for D in range (len(self.datainfolist)):
            
            caddress=self.datainfolist[D].CTFaddress
            s=(caddress).find('\P')
            e=str(caddress).find('_CT')
            Case=int(caddress[s+2:e])
            CTType=caddress[e+1:e+4]
            if datalist:
                try:
                    if datalist[Case]:
                        try:
                            if datalist[Case][CTType]:
                                # print(datalist[Case][CTType])
                                datalist[Case][CTType].append(self.datainfolist[D])
                            else:
                                datalist[Case][CTType]=[self.datainfolist[D]]
                        except:
                            datalist[Case][CTType]=[self.datainfolist[D]]
                    else:
                        datalist[Case]={CTType:[self.datainfolist[D]]}
    ...
    def getCase(self,Case,CTType,idx,OldDict=False):

                
        # self.Case=self.datainfolist[idx]
        self.Case=self.datalist[Case][CTType][idx]
        # print(self.Case)
        #print(self.Case[0])
        # s=str(i).find('\P')
        # e=str(i).find('_CT')
        ...
        print('\n CTF \n')
        print(self.Case.CTFaddress)
        print('\n SPF \n')
        print(self.Case.SPFaddress)
        proji, projl, cto, spo=getvaldata(self.Case)

        if OldDict:
            self.proji=np.zeros_like(proji)
            self.proji[0,:,:]=proji[0,:,:]
            self.proji[1,:,:]=proji[2,:,:]
            self.proji[2,:,:]=proji[4,:,:]
            self.proji[3,:,:]=proji[1,:,:]
            self.proji[4,:,:]=proji[3,:,:]
            self.proji[5,:,:]=proji[5,:,:]

            self.proji[3,:,:]=self.proji[3,:,:]/self.proji[3,:,:].max()
            self.proji[4,:,:]=self.proji[4,:,:]/self.proji[4,:,:].max()
            self.proji[5,:,:]=self.proji[5,:,:]/self.proji[5,:,:].max()
    ...
    def doValidation(self,dictaddress, useLFOV=False):
        ...
        self.model.netP.load_state_dict(torch.load(dictaddress[0]))
        self.model.netG.load_state_dict(torch.load(dictaddress[1]))

        self.model.netP.eval()
        self.model.netG.eval()
        # self.x=torch.cat(self.projc)
        self.xs=torch.from_numpy(self.proji).unsqueeze(0).to(self.device)
        self.xl=torch.from_numpy(self.projl).unsqueeze(0).to(self.device)
        # print(self.xs.shape)
        with torch.no_grad():
            
            self.xlout=self.model.netP.forward(self.xs).squeeze(0)
            self.Lout=self.xlout.cpu().detach().numpy()
            # print(XV.shape)
            XV1=self.xlout[0,:,:].unsqueeze(0).unsqueeze(0).to(self.device)
            XV2=self.xlout[1,:,:].unsqueeze(0).unsqueeze(0).to(self.device)
            XV3=self.xlout[2,:,:].unsqueeze(0).unsqueeze(0).to(self.device)
            
            if useLFOV:
                XV1=self.xl.squeeze(0)[0,:,:].unsqueeze(0).unsqueeze(0).to(self.device)
                XV2=self.xl.squeeze(0)[1,:,:].unsqueeze(0).unsqueeze(0).to(self.device)
                XV3=self.xl.squeeze(0)[2,:,:].unsqueeze(0).unsqueeze(0).to(self.device)


            out = self.model.netG.forward([XV1,XV2,XV3])
            
        self.out1=out[0].cpu().detach().squeeze(0).numpy()
        self.out2=out[1].cpu().detach().squeeze(0).numpy()
        self.out3=out[2].cpu().detach().squeeze(0).numpy()
        self.out4=out[3].cpu().detach().squeeze(0).numpy()
        # self.out=out.numpy()
        # self.out=out.numpy()
