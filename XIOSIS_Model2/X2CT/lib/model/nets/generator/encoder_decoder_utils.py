...
class Transposed_And_Add(nn.Module):
  def __init__(self, view1Order, view2Order, view3Order, sortOrder=None):
    ...
    self.permuteView3 = tuple(np.argsort(view3Order))

  def forward(self, *input):
    # return tensor in order of sortOrder
    return (input[0].permute(*self.permuteView1) + input[1].permute(*self.permuteView2) + input[2].permute(*self.permuteView3))