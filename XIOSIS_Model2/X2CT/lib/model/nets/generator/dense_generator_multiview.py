...
class MultiView_UNetLike_DenseDimensionNet(nn.Module):
  def __init__(self, view1Model, view2Model, view3Model, view1Order, view2Order, view3Order, backToSub, decoder_output_channels, decoder_out_activation, decoder_block_list=None, decoder_norm_layer=nn.BatchNorm3d, upsample_mode='nearest'):
    super(MultiView_UNetLike_DenseDimensionNet, self).__init__()
    self.view1Model = view1Model
    self.view2Model = view2Model
    self.view3Model = view3Model
    self.view1Order = view1Order
    self.view2Order = view2Order
    self.view3Order = view3Order
    ...
    self.transposed_layer = Transposed_And_Add(view1Order, view2Order, view3Order)
  ...
  def forward(self, input):
    ...
    # View 3 encoding process
    view3_encoder_feature = self.view3Model.encoder_layer(input[2])
    view3_next_input = view3_encoder_feature
    for i in range(self.view3Model.n_downsampling):
      setattr(self.view3Model, 'feature_linker' + str(i),
              getattr(self.view3Model, 'linker_layer' + str(i))(view3_next_input))
    ...
    # View 3 decoding process Part1
    view3_next_input = self.view3Model.base_link(view3_next_input.view(view3_next_input.size(0), -1))
    view3_next_input = view3_next_input.view(view3_next_input.size(0), self.view3Model.decoder_channel_list[-1], self.view3Model.decoder_begin_size,
                                             self.view3Model.decoder_begin_size, self.view3Model.decoder_begin_size)

    view_next_input = None
    # View 1 and 2 and 3 decoding process Part2
    for i in range(self.n_downsampling - 1, -2, -1):
      if i == (self.n_downsampling - 1):
        ...
        if self.backToSub:
          view_avg = self.transposed_layer(view1_next_input, view2_next_input, view3_next_input) / 3
          # print ('first', view_avg.shape)
          # print (view1_next_input.shape)
          # print (view2_next_input.shape)
          # print (view3_next_input.shape)
          view1_next_input = view_avg.permute(*self.view1Order)
          view2_next_input = view_avg.permute(*self.view2Order)
          view3_next_input = view_avg.permute(*self.view3Order)
        ...
        view3_next_input = getattr(self.view3Model, 'decoder_layer' + str(i))(view3_next_input)
      else:
        ...
        view3_next_input = getattr(self.view3Model, 'decoder_compress_layer' + str(i))(torch.cat((view3_next_input, getattr(self.view3Model, 'feature_linker' + str(i + 1))), dim=1))
        ########### MultiView Fusion
        # Method One: Fused feature back to sub-branch
        if self.backToSub:
          view_avg = self.transposed_layer(view1_next_input, view2_next_input, view3_next_input) / 3
          # print ('second',view_avg.shape)
          # print (view1_next_input.shape)
          # print (view2_next_input.shape)
          # print (view3_next_input.shape)
    ...
    return self.view1Model.decoder_layer(view1_next_input), self.view2Model.decoder_layer(view2_next_input), self.view3Model.decoder_layer(view3_next_input), self.decoder_layer(view_next_input)